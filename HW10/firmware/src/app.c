...
void APP_Tasks(void) {
    ...
    switch (appData.state) {
        ...
        case APP_STATE_SCHEDULE_WRITE:
            ...
            i = 0;
            int m = 0;
            float maf[4], maf_average, maf_sum = 0;
            float iir, fir;
            float i1 = 0.65, i2 = 0.35;
            float f1 = 0.0102, f2 = 0.1177, f3 = 0.3721, f4 = 0.3721, f5 = 0.1177, f6 = 0.0102;
            static int buff[2], buff2[6];
            for(m=0;m<2;m++) {buff[m] = 0;}
            for(m=0;m<6;m++) {buff2[m] = 0;}
            _CP0_SET_COUNT(0);
            for (i = 0; i<100; i++) {
                ...
                //MAF filter
                for(k=2;k>=0;k--) {maf[k+1]=maf[k];}
                maf[0] = signed_data[6];
                for (k=0;k<4;k++) {
                    maf_sum = maf_sum + maf[k];
                }
                maf_average = maf_sum/4;
                maf_sum = 0;
                
                //IIR Filter
                buff[1] = buff[0];
                buff[0] = signed_data[6];
                iir = (float) i1*buff[0]+i2*buff[1];
                //FIR filter
                for(k=4;k>=0;k--) {buff2[k+1]=buff2[k];}
                buff2[0]=signed_data[6];
